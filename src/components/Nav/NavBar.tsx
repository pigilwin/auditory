        ...
        <nav className="flex items-center justify-between flex-wrap bg-teal-500 p-6">
            <div className="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
                <div className="lg:flex-grow text-center lg:text-left">
                    <h1 className="text-3xl dark:text-white">Sounds</h1>
                </div>
                <GoBack/>
...
const NavButton = ({id, title, to}: NavButtonProps): JSX.Element => {
    return (
        <Link to={to} id={id} className="block mt-4 lg:inline-block lg:mt-0 text-black dark:text-white cursor-pointer hover:text-blue-400 dark:hover:text-blue-400 mr-4 text-lg">
            {title}
        </Link>
    );
}